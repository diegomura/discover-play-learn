...
class ScatteredChart {
  constructor({ title, width = 1200, height = 600, xAxis, yAxis, style = defaultStyle }) {
    this.title = title
    this.width = width
    this.height = height
    this.xAxis = xAxis
  ...
  #renderBackground() {
    return this.#renderRect({ x: 0, y: 0, width: this.width, height: this.height, fill: 'white', stroke: "none" })
  }

  #renderTitle() {
    if (!this.title) return ''

    return this.#renderLabel({ x: '50%', y: 25, fontSize: 24, text: this.title })
  }
  ...
  #renderXAxis() {
    ...
    axis += `<polygon
      fill="black"
      points="${this.width},${xAxisY} ${this.width - 8},${xAxisY - 8} ${this.width - 8},${xAxisY + 8}"
    />`
  ...
  #renderYAxis() {
    ...
    axis += this.#renderLine({ x1: yAxisX, y1: this.height, x2: yAxisX, y2: 0 })
  ...
  render() {
    ...
    const chartBox = this.#renderRect({ x: 0, y: 0, width: this.width, height: this.height })
    ...
    this.points.forEach((point) => {
      ...
      content += this.#renderCircle({ cx, cy, radius, fill: point.fill || 'black' })
    })
